document.addEventListener('DOMContentLoaded', () => {
    ...
    const data = {
            ...
            resolutionImpact: { "1080p": { gpu: 1.0 }, "1440p": { gpu: 0.85 }, "4k": { gpu: 0.65 }, "ultrawide": { gpu: 0.9 } }
        };
    ...
    function calculateBottleneck() {
        const selected = {
            ...
            ram: dom.ramSelect.value,
            resolution: dom.resolutionSelect.value, 
            purpose: dom.purposeSelect.value
        };

        if (!selected.cpu || !selected.gpu || selected.cpu === "-- Select CPU --" || selected.gpu === "-- Select GPU --") {
    ...
    function displayResults(selected, bottleneck, cpuInfo, gpuInfo) {
